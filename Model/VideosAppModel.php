 ...
 */
	protected function _validateVideoFile($data, $field, $modelAlias, $colom, $index = 0) {
		//更新用：ファイル更新の準備
 ...
 */
	protected function _saveVideoFile($data, $field, $modelAlias, $colom, $index = 0) {
		//更新用：ファイル削除
		$data = $this->_deleteFile($data, $modelAlias, $colom, $index);
 ...
 * @throws InternalErrorException
 */
	protected function _deleteFile($data, $modelAlias, $colom, $index = 0) {
		if (isset($data['DeleteFile'][$index]['File']['id']) && $data['DeleteFile'][$index]['File']['id'] > 0) {
